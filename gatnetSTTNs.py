...
class GraphAttentionLayer(nn.Module):
    def __init__(self, in_c, out_c):
        ...
        #self.a = nn.Parameter(torch.Tensor(size=(2*out_c, 1)))
        self.a = nn.Linear(2*out_c, 1, bias=False)
        self.concat = True
    ...
    #     #print(self.F(self.Leaky(outputs),dim=2))
    #     #print((outputs))
    ...
    def forward(self, inputs, graph):
        ...
        a_input = torch.cat([h.repeat(1,1,N).view(B,N*N, -1), h.repeat(1,N,1).view(B,N*N,-1)], dim=2).view(B,N,-1, 2 * out_Size) # shape[B,N, N, 2*out_features]
        #e = self.leakyrelu(torch.matmul(a_input, self.a).squeeze(3))  # [B,N,N,1] -> [B,N,N]
        e = self.leakyrelu(self.a(a_input).squeeze(3))  # [B,N,N,1] -> [B,N,N]
        #print(e)
        zero_vec = 0*torch.ones_like(e)
        attention = torch.where(graph > 0, e, zero_vec)
        
        attentiona = F.softmax(attention, dim=1).transpose(1, 2)
        attentionb = F.softmax(attention, dim=2)
        attentionc = torch.matmul(attentiona,attentionb)
        #print(attention)
        #attention = F.dropout(attention, self.dropout, training=self.training)
        h_prime = torch.matmul(attentionb, h) + self.b # [N,N], [N, out_features] --> [N, out_features]
...
class GATNet(nn.Module):
    ...
    def forward(self, data,graph, device):
       ...
       """
        ...
        prediction = self.subnet(data, graph).unsqueeze(2)  # [B, N, 1, C]，这个１加上就表示预测的是未来一个时刻
