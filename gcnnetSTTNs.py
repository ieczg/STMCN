...
class GCN(nn.Module):  # GCN模型，向空域的第一个图卷积
    ...
    def forward(self, data,graph_data):
        #graph_data = data["graph"].to(device)[0]  # [N, N] 邻接矩阵，并且将数据送入设备
        graph_data2 = GCN.process_graph(graph_data)  # 变换邻接矩阵 \hat A = D_{-1/2}*A*D_{-1/2}
        ...
        # 第一个图卷积层 特征变换过程中添加的可训练参数
        output_1 = self.linear_1(data)  # [B, N, hid_C],这个就是 WX，其中W是可学习的参数，X是输入的流量数据（就是flow_x）
        output_1 = self.act(torch.matmul(graph_data2, output_1))  # [B, N, N] ,[B, N, hid_c]，就是 \hat AWX

        # 第二个图卷积层
        output_2 = self.linear_2(output_1)# WX
        output_2 = self.act(torch.matmul(graph_data2, output_2))  # [B, N, 1, Out_C] , 就是 \hat AWX
